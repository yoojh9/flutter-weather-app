...
Future<dynamic> getKorCurrentWeather(int x, int y) async {
  ...
  final urlStr =
      'http://apis.data.go.kr/1360000/VilageFcstInfoService/getUltraSrtFcst?serviceKey=$apiKey&pageNo=1&numOfRows=100&dataType=JSON&base_date=$baseDate&base_time=$baseTime&nx=$x&ny=$y';
...
Future<dynamic> getKorMaxMinTemp(int x, int y) async {
  ...
  if (baseTime < 2) {
    baseDate =
        DateFormat('yyyyMMdd').format(dateTime.subtract(Duration(days: 1)));
  }

  final urlStr =
      'http://apis.data.go.kr/1360000/VilageFcstInfoService/getVilageFcst?serviceKey=$apiKey&pageNo=1&numOfRows=100&dataType=JSON&base_date=$baseDate&base_time=0200&nx=$x&ny=$y';
  final url = Uri.parse(urlStr);
