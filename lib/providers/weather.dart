...
class Weather with ChangeNotifier {
  ...
  Future<void> getWeather() async {
    if (_locationInfo.latitude == null || _locationInfo.longitude == null)
      return;
  ...
  Future<void> setWeatherDataFromKorWeather(locationInfo) async {
    List currentList = await korWeatherAPI.getKorCurrentWeather(
        locationInfo.x, locationInfo.y);
    List minMaxTempList =
        await korWeatherAPI.getKorMaxMinTemp(locationInfo.x, locationInfo.y);
    final forecast = await openWeatherAPI.getOpenWeatherAPI(
  ...
  Future<void> setWeatherDataFromOpenWeatherMap(locationInfo) async {
    var body = await openWeatherAPI.getOpenWeatherAPI(
        locationInfo.latitude, locationInfo.longitude);
   ...
   * 기상청 동네예보 API 사용
   */
  void setCurrenWeatherCodeWithKorWeather(
      List currentItemList, final forecast) {
    ...
    final tempItem =
        currentItemList.firstWhere((item) => item['category'] == "T1H");
    currentWeather.temp = int.parse(tempItem['fcstValue']);

    final skyItem =
        currentItemList.firstWhere((item) => item['category'] == "SKY");
    final ptyItem =
        currentItemList.firstWhere((item) => item['category'] == "PTY");
    ...
    switch (skyItem['fcstValue']) {
      case "1":
        ...
        currentWeather.icon = "01";
        break;
      case "3":
        currentWeather.id = 803;
        currentWeather.icon = "03";
        break;
      case "4":
        currentWeather.id = 804;
        currentWeather.icon = "04";
        break;
      default:
        currentWeather.id = 800;
        currentWeather.icon = "01";
    ...
    switch (ptyItem['fcstValue']) {
      case "0":
        break;
      case "1":
        currentWeather.id = 501;
        currentWeather.icon = "09";
        break;
      case "2":
      case "6":
        currentWeather.id = 611;
        currentWeather.icon = "13";
        break;
      case "3":
      case "7":
        currentWeather.id = 601;
        currentWeather.icon = "13";
        break;
      case "4":
        currentWeather.id = 505;
        currentWeather.icon = "09";
        break;
      case "5":
        currentWeather.id = 500;
        currentWeather.icon = "10";
        break;
      default:
        break;
    }
    ...
    // 일몰 일출 시간에 따라 아이콘 변경
    if (dateTime.isAfter(currentWeather.sunriseDateTime) &&
        dateTime.isBefore(currentWeather.sunsetDateTime)) {
  ...
  void setMinMaxTempWithKorWeather(List minMaxTempList) {
    ...
    final minItem =
        minMaxTempList.firstWhere((item) => item['category'] == "TMN");
    final maxItem =
        minMaxTempList.firstWhere((item) => item['category'] == "TMX");
